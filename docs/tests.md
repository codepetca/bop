...
- **GameEngine:** start/reset state, next command selection, timeout-only failure, scoring and high-score updates, difficulty ramp (start ~3s, -0.1s every 3 successes, floor 0.5s) with speed-up cue trigger.
...
2) **GameEngine (tests first):**
   - `startGame` resets state, sets time to `GameConstants.initialTimePerCommand` (~3s), picks a command.
